    ...
    if (tabBtns.length > 0) {
        ...
        const generateClassStudents = (className) => {
            ...
            const class3B_Students = [
                ...
                'Rafael dos Santos de Lima', 'Rafaela Silva Correa', 'Rita de Cassia dos Santos Santana',
            ...
            storedData.sort((a, b) => a.name.localeCompare(b.name, 'pt-BR'));
            // Atualiza os números de chamada conforme a nova ordem
            storedData.forEach((student, index) => {
                student.number = index + 1;
            });
            // Salva a ordem atualizada no localStorage
            localStorage.setItem(storageKey, JSON.stringify(storedData));
            return storedData;
        };
        ...
        tabBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // Se clicar na aba reservas, renderiza as permissões e a tabela
        ...
        const reservationTabBtn = document.querySelector('.tab-btn[data-tab="reservations"]');
        const calendarDatePicker = document.getElementById('calendarDatePicker');
        const fabianaAddReserva = document.getElementById('fabianaAddReserva');
        const fabianaProfessor = document.getElementById('fabianaProfessor');
        const fabianaTurno = document.getElementById('fabianaTurno');
        const fabianaAula = document.getElementById('fabianaAula');
        const fabianaAddReservaBtn = document.getElementById('fabianaAddReservaBtn');

        let currentDate = null;

        // Função para verificar se o usuário tem permissão para ver a aba de reservas
        const isReservationUser = (profile) => {
            return profile && (profile.name === 'Delia' || profile.name === 'Antônio' || profile.userType === 'bibliotecaria');
        };

        // Função para verificar se o usuário é Fabiana (pode adicionar/excluir reservas)
        const isFabiana = (profile) => {
            return profile && profile.userType === 'bibliotecaria';
        };

        // Função para obter o número de aulas por turno
        const getNumAulasPerTurno = (turno) => {
            if (turno === 'manha') return 6;
            return 5;
        };

        // Função para obter o nome do turno formatado
        const getTurnoName = (turnoValue) => {
            const turnos = {
                'manha': 'Manhã',
                'tarde': 'Tarde',
                'noite': 'Noite'
            };
            return turnos[turnoValue] || turnoValue;
        };

        // Função para obter o dia da semana a partir de mês e dia
        const getDayOfWeek = (month, day) => {
        ...
        // Função para preencher aulas select do formulário
        const fillAulasSelect = (turno, selectElement) => {
            if (!selectElement) return;
            selectElement.innerHTML = '<option value="">Selecione a aula</option>';
            const numAulas = getNumAulasPerTurno(turno);
            for (let i = 1; i <= numAulas; i++) {
                const option = document.createElement('option');
                option.value = i;
                option.textContent = `Aula ${i}`;
        ...
        const renderReservationsTable = () => {
            ...
            const isFab = isFabiana(userProfile);
            
            // Show/hide add section for Fabiana
            if (fabianaAddReserva) {
                fabianaAddReserva.style.display = isFab ? 'block' : 'none';
            }
            
            if (!currentDate) {
                if (reservationsTableBody) {
                    reservationsTableBody.innerHTML = '<tr><td colspan="4" style="text-align: center; color: var(--text-muted); padding: 20px; font-style: italic;">Selecione uma data para ver as reservas</td></tr>';
                }
                return;
            }

            const storageKey = 'reservations_shared';
            ...
            const filteredReservations = reservations.filter(
                r => r.month === monthPart && r.day === parseInt(day)
            );
            ...
            if (reservationsTableBody) {
                ...
                if (filteredReservations.length === 0) {
                    reservationsTableBody.innerHTML = '<tr><td colspan="4" style="text-align: center; color: var(--text-muted); padding: 20px; font-style: italic;">Nenhuma reserva para essa data</td></tr>';
                    return;
                }

                // Ordenar por turno e aula
                filteredReservations.sort((a, b) => {
                    const turnoOrder = ['manha', 'tarde', 'noite'];
                    const turnoDiff = turnoOrder.indexOf(a.turno) - turnoOrder.indexOf(b.turno);
                    if (turnoDiff !== 0) return turnoDiff;
                    return a.aula - b.aula;
                });

                filteredReservations.forEach((reservation, index) => {
                    const originalIndex = reservations.findIndex(r => r.createdAt === reservation.createdAt);
                    const row = document.createElement('tr');
                    row.style.borderBottom = '1px solid var(--border-color)';
                    row.innerHTML = `
                        <td style="padding: 12px 16px; color: var(--text-main);">${getTurnoName(reservation.turno)}</td>
                        <td style="padding: 12px 16px; color: var(--text-main);">${reservation.aula}</td>
                        <td style="padding: 12px 16px; color: var(--text-main);">${reservation.professor}</td>
                        <td style="padding: 12px 16px;">
                            ${isFab ? `
                                <button class="btn-delete-reservation" data-index="${originalIndex}" style="padding: 8px 12px; border: none; border-radius: 8px; background: rgba(239, 68, 68, 0.1); color: #ef4444; cursor: pointer; transition: background 0.2s;">
                ...
                document.querySelectorAll('.btn-delete-reservation').forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        const index = parseInt(e.target.closest('.btn-delete-reservation').dataset.index);
        ...
        // Função para renderizar as permissões da aba de reservas
        const renderReservations = () => {
            const userProfile = JSON.parse(localStorage.getItem('userProfile'));
            
            // Mostra/oculta a aba de reservas
            if (reservationTabBtn) {
                reservationTabBtn.style.display = isReservationUser(userProfile) ? 'flex' : 'none';
            }
        };
        ...
        const deleteReservation = (index) => {
            ...
            localStorage.setItem(storageKey, JSON.stringify(reservations));
            renderReservationsTable();
            showToast('Reserva Removida', 'A reserva foi cancelada com sucesso.', 'info');
        };

        // Evento de mudança no turno do formulário de Fabiana
        if (fabianaTurno) {
            fabianaTurno.addEventListener('change', (e) => {
                const turno = e.target.value;
                fillAulasSelect(turno, fabianaAula);
            });
        }
        ...
        // Fabiana's add reservation button
        if (fabianaAddReservaBtn) {
            fabianaAddReservaBtn.addEventListener('click', () => {
                const turno = fabianaTurno.value;
                const aula = fabianaAula.value;
                const professor = fabianaProfessor.value;
                
                if (!currentDate) {
                    showToast('Erro', 'Por favor, selecione uma data.', 'error');
                    return;
                }
                if (!turno || !aula || !professor) {
                    showToast('Erro', 'Por favor, preencha todos os campos.', 'error');
                    return;
                }
                
                // Split date into YYYY-MM and day
                const [year, month, day] = currentDate.split('-');
                const monthPart = `${year}-${month}`;
                const dayWeek = getDayOfWeek(monthPart, day);
                ...
                // Clear form
                fabianaTurno.value = '';
                fabianaAula.innerHTML = '<option value="">Selecione o turno primeiro</option>';
                fabianaProfessor.value = '';
                showToast('Reserva Feita!', 'Reserva confirmada com sucesso!', 'success');
            });
        }

        // Atualiza reservas quando houver mudanças no localStorage (outra aba/janela)
        window.addEventListener('storage', (e) => {
            if (e.key === 'reservations_shared') {
        ...
        if (editProfileBtn && editModal) {
            ...
            if (savedProfile) {
                ...
                if (savedProfile.userType === 'teacher') {
                    ...
                    // Set active tab to Biblioteca for Fabiana
                    ...
                    const libraryTabPane = document.getElementById('library');
                    
                    if (homeTabBtn) homeTabBtn.classList.remove('active');
                    if (homeTabPane) homeTabPane.style.display = 'none';
                    if (libraryTabBtn) libraryTabBtn.classList.add('active');
                    if (libraryTabPane) libraryTabPane.style.display = 'block';
                } else {
